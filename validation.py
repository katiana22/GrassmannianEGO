                ...
                if om.distanceMetric == 0:
                    ...
                    if self.reference.fromContinuum:
                        ...
                        for i in np.arange(0, om.nContinuumFrames):
                            ...
                            stress_cm[i] = self.parameters.getValue('s_y')*computeContinuumDeviatoricStress(om.continuumOutPath, i)

                            ix = np.argmin(np.abs(self.reference.strain-strain_cm[i]))
                            distance_i[i] = np.abs(1.00556564*self.reference.stress[ix]-stress_cm[i])
...
def ego(optimizationModel, reference, initParams, checkPrevious=True, 
    ...
    while len(Y) < optimizationModel.nTotalObs and Y_best > optimizationModel.dtol:
        ...
        if iObs < optimizationModel.nInitialObs:
            ...
            # Generate surrogant points by Sobol sequencing 
            A = []
            for param in initParams.parameters:
                if not param.is_constant:
                    A.append(cp.Uniform(param.vmin, param.vmax))
            distA = cp.J(*A)
            X_surr = distA.sample(om.nSurrEvals, rule='S')
            X_surr = X_surr.T
            ...
            if normalize:
                gp = optimizationModel.trainModel(Xs_, Ys)
                Y_surr, s2 = gp.predict(X_surr_, return_std=True)
                
            else:
                gp = optimizationModel.trainModel(Xs, Ys)
                Y_surr, s2 = gp.predict(X_surr, return_std=True)
            ...
            plotConvergence(X, Y, initParams, om, targetVals)
...
def plotComparison(parameters, optimizationModel, reference, X, Y):
    ...
    if reference.fromContinuum:
        ...
        for it in np.arange(0, optimizationModel.nContinuumFrames):
            ...
            tau_Ref[it] = 1.00556564*computeContinuumDeviatoricStress(\
...
def plotConvergence(X, Y, parameters, optimizationModel, targetVals, nPanes=4):
            ...
            if not param.is_constant:
                ...
                if np.floor(ip/nPanes) == iFig:
                    ...
                    if param.name in targetVals:
                        plt.axhline(y=targetVals[param.name], color='red', \
                            linestyle='--', linewidth=3, label='target value')                          
...
def main():
    ...
    om = OptimizationModel(nInitialObs=400, nTotalObs=3500, nSurrEvals=1500000, \
                            samplingstyle=3,\
    ...
    beta = Parameter('beta', 8.27674103 , vmin=2, vmax=15)
    u0 = Parameter('u0', -3.36821766, vmin=-3.390, vmax=-3.355)
    chi_len = Parameter('chi_len', 41.79654694)
    ep = Parameter('ep', 7.12963104)
    c0 = Parameter('c0', 0.54989662, vmin=0.05, vmax=1)
    chi_inf = Parameter('chi_inf', 2806)
    s_y = Parameter('s_y', 1.00556564, vmin=0.8, vmax=1.15)
